...
public:
    template <typename Callable, bool = std::is_invocable_r_v<R, Callable&&, Args...>>
    function_ref(Callable&& c) noexcept :
        obj_(std::addressof(c)), cb_([](void* obj, Args... args) {
            return std::invoke(*static_cast<std::add_pointer_t<Callable&&>>(obj), std::forward<Args>(args)...);
        }) {}
